...
class _StarFieldState extends State<StarField>
  ...
  // ValueNotifier strictly for triggering repaints without rebuilding the widget tree
  ...
  void initState() {
    ...
    // Use a native Ticker for a continuous, highly efficient game loop
  ...
  void _onTick(Duration elapsed) {
    ...
    for (final s in _stars) {
      ...
      double x = s.pos.dx;
      double y = s.pos.dy;
      if (x < -s.radius * 2) x = w + s.radius * 2;
      if (x > w + s.radius * 2) x = -s.radius * 2;
      if (y < -s.radius * 2) y = h + s.radius * 2;
      if (y > h + s.radius * 2) y = -s.radius * 2;
      s.pos = Offset(x, y);
    ...
    // Trigger ONLY the CustomPainter to update, not the whole widget
  ...
  void _maybeGenerateStars(Size size) {
    ...
    _stars = List.generate(count, (i) {
      ...
      // Radius generation
      final isBigStar = _rng.nextDouble() < 0.15;
      final radius = (_rng.nextDouble() * 1.8) + (isBigStar ? 1.8 : 0.2);

      // PARALLAX: Bind speed directly to radius (bigger stars look closer, so they move faster)
      final depthMultiplier = radius / 3.6;
      final baseSpeed = (_rng.nextDouble() * 20) + 10;
      final speed = baseSpeed * (0.5 + depthMultiplier);

      final angle = _rng.nextDouble() * math.pi * 2;
      final velocity = Offset(
        math.cos(angle) * speed,
        math.sin(angle) * speed * 0.6, // slight horizontal bias
      );
...
class _StarPainter extends CustomPainter {
  ...
  _StarPainter({
    ...
    required this.glowIntensity,
            ...
            repaint:
                repaintNotifier); // Passes the notifier to the superclass to handle repaints efficiently
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final now = repaintNotifier.value;
    ...
    for (final s in stars) {
      ...
      // PERFORMANCE BOOST: Replaced MaskFilter.blur with RadialGradient for the glow
      if (glowIntensity > 0.0) {
        final glowRadius = s.radius * 3.0;
        final glowPaint = Paint()
          ..shader = RadialGradient(
            colors: [
              s.color.withOpacity((alpha * glowIntensity).clamp(0.0, 1.0)),
              s.color.withOpacity(0.0),
            ],
          ).createShader(Rect.fromCircle(center: s.pos, radius: glowRadius));

        canvas.drawCircle(s.pos, glowRadius, glowPaint);
      }

      // Draw the sharp core of the star
      final corePaint = Paint()..color = s.color.withOpacity(alpha);
      canvas.drawCircle(s.pos, s.radius, corePaint);
    }
  }
